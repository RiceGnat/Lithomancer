...
namespace Lithomancer.MapGeneration.Maps
{
	...
	public class MineMap : IMineMap
	{
		...
		private readonly HashSet<(int X, int Y)> doorPoints = new HashSet<(int X, int Y)>();

		public MineMap(int seed, CaveParameters caveParameters, MineParameters mineParameters, IEnumerable<OreParameters> ores)
		{
			Random random = new Random(seed);
			Seed = seed;

			int x = caveParameters.Size.X;
			int y = caveParameters.Size.Y;

			cave = new CaveTerrain(random.Next(), caveParameters);

			Doors = SelectDoors(random.Next(), mineParameters.DoorCount, mineParameters.DoorSize, mineParameters.DoorDistance, MainRegion.Border);
			foreach (Segment door in Doors)
			{
				foreach (Pixel p in door.Pixels)
				{
					doorPoints.Add(p);
				}
			}

			IHeightMap rocks = GenerateRocks(random.Next(), x, y, mineParameters.RockDensity);
			int height = rocks.MaxHeight + 1;
			Terrain = new HeightMap(height, ValueArray.Create(x, y, (i, j) => {
				if (cave[i, j] > 0) return height;
				else if (Doors.Count > 0 && Doors[0].Center.DistanceTo((i, j)) <= mineParameters.ClearEntranceRadius) return 0;
		...
		public bool IsDoor(int x, int y) => doorPoints.Contains((x, y));
	}
}
